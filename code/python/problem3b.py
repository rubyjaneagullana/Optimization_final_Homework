"""Worst-case sidelobe minimization for a 40-element ULA."""
...
t = cp.Variable()  # epigraph variable representing the max sidelobe power

# Preserve unity gain toward ANGLE_DES to avoid distorting the desired signal.
constraints = [cp.sum(cp.multiply(np.conj(Steering_des), w)) == 1]

# Explicitly constrain every blocked angle so that |wᴴa(θ)|² ≤ t.
...
# Minimize the worst-case sidelobe value captured by t.
...
# Evaluate wᴴa(θ) power over the full ±90° scan to visualize worst-case behavior.
...
ax.set_xlim([-90, 90])
fig.savefig("problem3b.png", dpi=300, bbox_inches="tight")
with open("problem3b.fig.pickle", "wb") as fig_file:
