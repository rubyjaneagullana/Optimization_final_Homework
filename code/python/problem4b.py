...
ANGLE_MS = np.array([-33, -78, 10, 64])
sigma_i = np.sqrt(0.01) * np.ones(NUM_MS)
gamma_dB = 10
...
sigma_column = cp.Constant(sigma_i.reshape(-1, 1))
...
for i, label in enumerate(
    [r"MS at $-33^\circ$", r"MS at $-78^\circ$", r"MS at $10^\circ$", r"MS at $64^\circ$"]
):
...
fig.savefig("problem4b.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close(fig)
